...
export class Player {
  ...
  dodge(enemies = []) {
    if (this.stamina >= 20 && this.dodgeTimer <= 0) {
      ...
      // Check for Perfect Dodge (if nearby enemy is attacking within 85px)
      let isPerfect = false;
      enemies.forEach(en => {
  ...
  handleInput(keys, mousePos, camera, dt) {
    ...
    if (hasInput) {
      if (this.isSprinting) {
        ...
        targetSpeed = this.jogSpeed || this.speed;
    ...
    if (this.isMoving) {
      ...
      this.strideTime = (this.strideTime || 0) + dt * strideFreq;
      this.stridePhase = Math.sin(this.strideTime);
    } else {
      this.strideTime = (this.strideTime || 0) + dt * 2.0;
      this.stridePhase = (this.stridePhase || 0) * Math.pow(0.1, dt);
    }
    ...
    this.facingAngle += angleDiff * Math.min(1, (this.turnSpeed || 14) * dt);
  ...
  attack(enemies, particleEffects) {
    ...
    const baseDamage = 25;
    const comboMultipliers = { 1: 1.0, 2: 1.28, 3: 1.85 };
    const isCrit = this.comboStep === 3;
    const dodgeMul = this.perfectDodgeBoost || 1.0;
      ...
      if (enemy.isAlive) {
        ...
        if (dist <= attackRange) {
          ...
          if (angleDiff <= attackArc / 2) {
            ...
            hitAny = true;
            this.comboCounter = (this.comboCounter || 0) + 1;
            ...
            const knockbackDist = isCrit ? 48 : 26;
